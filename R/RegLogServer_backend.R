...
#'         # check if the inputs are filled
#'         if (!all(
#'           #isTruthy(input$register_user_ID),
#'           isTruthy(input$register_email),
#'           isTruthy(input$register_pass1),
#'           isTruthy(input$register_pass2))) {
#' 
#'           # TODO see if I need this later
#'           register_user_ID = register_email
#' 
#'           modals_check_n_show(private = private,
...
#'               !check_user_mail(input$register_email) ||
#'               !check_user_pass(input$register_pass1) ||
#'               (input$register_pass1 != input$register_pass2)) {
#' 
#'             # parse message to show
#'             message_to_show <- RegLogConnectorMessage(
#'               "register_front",
...
#'               #identical_email = input$register_user_ID == input$register_email,
...
#'             modals_check_n_show(
#'               private = private,
#'               modalname = if (!message_to_show$data$valid_email) "register_nonValidEmail"
...
#'               else if (!messsage_to_show$data$identical_email) "register_notIdenticalEmail"
#'             )
...
#'           } else {
#' 
#'             on.exit(blank_textInputs(inputs =
#'                                        c("register_user_ID",
#'                                          "register_email"),
...
#' 
#'         # check if the inputs are filled
#'         if (!all(isTruthy(input$reset_user_ID),
#'                  isTruthy(input$reset_code),
#'                  isTruthy(input$reset_pass1),
#'                  isTruthy(input$reset_pass2))) {
#' 
    ...
    function(input, output, session) {
      ...
      observeEvent(input$login_button, {
        ...
        if (!all(isTruthy(input$login_user_id), isTruthy(input$password_login))) {
          ...
          message_to_send <- RegLogConnectorMessage(
            type = "login",
            username = tolower(trimws(input$login_user_id, "both")), # CERP changed
            password = trimws(input$password_login, "both")
          )
      ...
      observeEvent(input$register_bttn, {
        ...
        if (!all(
          ...
          if (#!check_user_login(input$register_user_ID) || 
            ...
            message_to_send <- RegLogConnectorMessage(
              type = "register",
              username = tolower(trimws(input$register_email, "both")), # CERP changed
              password = trimws(input$register_pass1, "both"),
              email = tolower(trimws(input$register_email, "both")) # CERP changed
            )
      ...
      observeEvent(input$reset_confirm_bttn, {
        ...
        # check if the inputs are filled
        if (!all(isTruthy(input$reset_email), 
                 isTruthy(input$reset_code),
                 isTruthy(input$reset_pass1), 
                 isTruthy(input$reset_pass2))) {
          ...
          message_to_send <- RegLogConnectorMessage(
            "resetPass_confirm",
            username = input$reset_email,
            reset_code = input$reset_code,
            password = input$reset_pass1
          )
          
          blank_textInputs(c("reset_email", 
                             "reset_code", 
                             "reset_pass1", 
                             "reset_pass2"), 
                           session = session)
